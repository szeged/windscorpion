...
function recurive_node(depth)
{
  var node = '<tag' + index + ' id="' + index + '">text';
  var nodeEnd = '</tag' + index + '>';
  var nodeBody = '';
  if (depth < 5) {
    index ++;
    while (rand() < 150) {
       nodeBody = nodeBody + recurive_node(depth + 1);
    }
  }
  return node + nodeBody + nodeEnd;
}

function generate_xml()
{
  var src = '<?xml version="1.0"?><rootnode>\n';
  for (var i = 0; i < 190; i++)
    src = src + recurive_node(0) + '\n';
  return src + '</rootnode>\n';
}

// ~ 0.020 sec
var src = generate_xml()
//print(src)

var objDom = new XMLDoc(src);
// var node = objDom.selectNode("/tag0"); print(node.getText());
